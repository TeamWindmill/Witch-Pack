...
namespace External_Assets.LeanTween.Examples.Scripts
{
	public class TestingZLegacy : MonoBehaviour {
		...
		void Start () {
			...
			Framework.LeanTween.delayedCall(1f, cycleThroughExamples);
...
//		alphaExample();
		...
		void cycleThroughExamples(){
			if(exampleIter==0){
				...
				if(timingType==TimingType.VariableTimeScale){
					...
					Framework.LeanTween.cancel( descrTimeScaleChangeId );
				}
			}
			...
			Framework.LeanTween.delayedCall( gameObject, delayTime, cycleThroughExamples).setUseEstimatedTime(useEstimatedTime);
		...
		public void colorExample(){
			...
			Framework.LeanTween.color( lChar, new Color(1.0f,0.0f,0.0f,0.5f), 0.5f ).setEase(LeanTweenType.easeOutBounce).setRepeat(2).setLoopPingPong().setUseEstimatedTime(useEstimatedTime);
		}
		...
		public void customTweenExample(){
			...
			Framework.LeanTween.moveX( ltLogo, -10.0f, 0.5f ).setEase(customAnimationCurve).setUseEstimatedTime(useEstimatedTime);
			Framework.LeanTween.moveX( ltLogo, 0.0f, 0.5f ).setEase(customAnimationCurve).setDelay(0.5f).setUseEstimatedTime(useEstimatedTime);
		}
	
		public void moveExample(){
			Debug.Log("moveExample");
		
			Framework.LeanTween.move( ltLogo, new Vector3(-2f,-1f,0f), 0.5f).setUseEstimatedTime(useEstimatedTime);
			Framework.LeanTween.move( ltLogo, origin, 0.5f).setDelay(0.5f).setUseEstimatedTime(useEstimatedTime);
		}
		...
		public void rotateExample(){
			...
			Framework.LeanTween.rotate( ltLogo, new Vector3(0f,360f,0f), 1f).setEase(LeanTweenType.easeOutQuad).setOnComplete(rotateFinished).setOnCompleteParam(returnParam).setOnUpdate(rotateOnUpdate).setUseEstimatedTime(useEstimatedTime);
		}
		...
		public void scaleExample(){
			...
			Framework.LeanTween.scale( ltLogo, new Vector3(currentScale.x+0.2f,currentScale.y+0.2f,currentScale.z+0.2f), 1f ).setEase(LeanTweenType.easeOutBounce).setUseEstimatedTime(useEstimatedTime);
		}
		...
		public void updateValueExample(){
			...
			pass.Add("message", "hi");
			Framework.LeanTween.value( gameObject, (Action<float, object>)updateValueExampleCallback, ltLogo.transform.eulerAngles.y, 270f, 1f ).setEase(LeanTweenType.easeOutElastic).setOnUpdateParam(pass).setUseEstimatedTime(useEstimatedTime);
		}
		...
		public void delayedCallExample(){
			...
			Framework.LeanTween.delayedCall(0.5f, delayedCallExampleCallback).setUseEstimatedTime(useEstimatedTime);
		}
		...
		public void delayedCallExampleCallback(){
			...
			Framework.LeanTween.scale( ltLogo, new Vector3(currentScale.x-0.2f,currentScale.y-0.2f,currentScale.z-0.2f), 0.5f ).setEase(LeanTweenType.easeInOutCirc).setUseEstimatedTime(useEstimatedTime);
		}
		...
		public void alphaExample(){
			...
			GameObject lChar = GameObject.Find ("LCharacter");
			Framework.LeanTween.alpha( lChar, 0.0f, 0.5f ).setUseEstimatedTime(useEstimatedTime);
			Framework.LeanTween.alpha( lChar, 1.0f, 0.5f ).setDelay(0.5f).setUseEstimatedTime(useEstimatedTime);
		}
		...
		public void moveLocalExample(){
			...
			Framework.LeanTween.moveLocal( lChar, new Vector3(0.0f,2.0f,0.0f), 0.5f ).setUseEstimatedTime(useEstimatedTime);
			Framework.LeanTween.moveLocal( lChar, origPos, 0.5f ).setDelay(0.5f).setUseEstimatedTime(useEstimatedTime);
		}
		...
		public void rotateAroundExample(){
			...
			GameObject lChar = GameObject.Find ("LCharacter");
			Framework.LeanTween.rotateAround( lChar, Vector3.up, 360.0f, 1.0f ).setUseEstimatedTime(useEstimatedTime);
		}
		...
		public void punchTest(){
			Framework.LeanTween.moveX( ltLogo, 7.0f, 1.0f ).setEase(LeanTweenType.punch).setUseEstimatedTime(useEstimatedTime);
		}
	}
}
