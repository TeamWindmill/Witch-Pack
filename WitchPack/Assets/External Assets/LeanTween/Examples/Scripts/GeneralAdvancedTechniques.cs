...
namespace External_Assets.LeanTween.Examples.Scripts
{
	public class GeneralAdvancedTechniques : MonoBehaviour {
		...
		void Start () {
			...
			Framework.LeanTween.alpha( avatarRecursive, 0f, 1f).setRecursive(true).setLoopPingPong();
			Framework.LeanTween.alpha( avatar2dRecursive, 0f, 1f).setRecursive(true).setLoopPingPong();
			Framework.LeanTween.alpha( wingPersonPanel, 0f, 1f).setRecursive(true).setLoopPingPong();
			...
			Framework.LeanTween.value( avatarMove, 0f, (float)movePts.Length-1, 5f).setOnUpdate((float val)=>{
			...
			for(int i = 0; i < movePts.Length; i++)
				Framework.LeanTween.moveY( movePts[i].gameObject, movePts[i].position.y + 1.5f, 1f).setDelay(((float)i)*0.2f).setLoopPingPong();


			// move objects at a constant speed
			for(int i = 0; i < avatarSpeed.Length; i++)
				Framework.LeanTween.moveLocalZ( avatarSpeed[i], (i+1)*5f, 1f).setSpeed(6f).setEase(LeanTweenType.easeInOutExpo).setLoopPingPong(); // any time you set the speed it overrides the time value
	
			// move around a circle at a constant speed
			for(int i = 0; i < avatarSpeed2.Length; i++){
				Framework.LeanTween.moveLocal( avatarSpeed2[i], i == 0 ? circleSm : circleLrg, 1f).setSpeed(20f).setRepeat(-1);
			}
