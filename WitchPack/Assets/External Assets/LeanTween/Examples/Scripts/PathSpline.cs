...
namespace External_Assets.LeanTween.Examples.Scripts
{
	public class ExampleSpline : MonoBehaviour {
		...
		void Start () {
			...
			Framework.LeanTween.moveSpline( ltLogo2, spline.pts, 1f).setEase(LeanTweenType.easeInOutQuad).setLoopPingPong().setOrientToPath(true);

			LTDescr zoomInPath_LT = Framework.LeanTween.moveSpline(ltLogo2, new Vector3[]{Vector3.zero, Vector3.zero, new Vector3(1,1,1), new Vector3(2,1,1), new Vector3(2,1,1)}, 1.5f);
