...
namespace External_Assets.LeanTween.Examples.Scripts
{
	public class GeneralCameraShake : MonoBehaviour {
		...
		void bigGuyJump(){
			...
			Framework.LeanTween.moveY(avatarBig, height, 1f).setEase(LeanTweenType.easeInOutQuad).setOnComplete( ()=>{
				Framework.LeanTween.moveY(avatarBig, 0f, 0.27f).setEase(LeanTweenType.easeInQuad).setOnComplete( ()=>{
					Framework.LeanTween.cancel(gameObject);
				...
				**************/
					...
					LTDescr shakeTween = Framework.LeanTween.rotateAroundLocal( gameObject, Vector3.right, shakeAmt, shakePeriodTime)
						.setEase( LeanTweenType.easeShake ) // this is a special ease that is good for shaking
						.setLoopClamp()
						.setRepeat(-1);

					// Slow the camera shake down to zero
					Framework.LeanTween.value(gameObject, shakeAmt, 0f, dropOffTime).setOnUpdate( 
						(float val)=>{
				...
				********************/
					...
					Framework.LeanTween.delayedCall(2f, bigGuyJump);
				});
			});
			jumpIter += 5.2f;
		}
