...
namespace External_Assets.LeanTween.Examples.Scripts
{
	public class GeneralSequencer : MonoBehaviour {
		...
		public void Start(){
			...
			seq.append( Framework.LeanTween.moveY( avatar1, avatar1.transform.localPosition.y + 6f, 1f).setEaseOutQuad() );
			...
			seq.insert( Framework.LeanTween.alpha(star, 0f, 1f) );
			seq.insert( Framework.LeanTween.scale( star, Vector3.one * 3f, 1f) );

			// Rotate 360
			seq.append( Framework.LeanTween.rotateAround( avatar1, Vector3.forward, 360f, 0.6f ).setEaseInBack() );

			// Return to ground
			seq.append( Framework.LeanTween.moveY( avatar1, avatar1.transform.localPosition.y, 1f).setEaseInQuad() );
			...
			seq.append(() => {
				for(int i = 0; i < 50f; i++){
					...
					Framework.LeanTween.moveLocal(cloud, range, 3f*speedScale).setEaseOutCirc();
					Framework.LeanTween.rotateAround(cloud, Vector3.forward, 360f*2, 3f*speedScale).setEaseOutCirc();
					Framework.LeanTween.alpha(cloud, 0f, 3f*speedScale).setEaseOutCirc().setDestroyOnComplete(true);
				}
			});
